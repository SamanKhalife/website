...
کنترل رفتار سرور API Kubernetes در شرایط آبشاری یک وظیفه کلیدی برای مدیران خوشه است. سرور kube-apiserver دارای برخی کنترل‌های موجود است
(مانند دستورالعمل‌های خط فرمان `--max-requests-inflight` و `--max-mutating-requests-inflight`)
برای محدود کردن مقدار کار ناموجود که پذیرفته می‌شود، جلوگیری از سیلاب کردن درخواست‌های ورودی و احتمالاً باعث کرش سرور API می‌شود، اما این پرچم‌ها کافی نیستند تا اطمینان حاصل شود که درخواست‌های مهم‌ترین در یک دوره ترافیک بالا عبور کنند.

ویژگی اولویت و عدالت API (APF) یک جایگزین است که بهبودی بر نقاط ضعف گفته شده در محدودیت‌های حداکثر عملکرد برقرار می‌کند. APF در یک روش دقیق‌تر، درخواست‌ها را دسته‌بندی و جدا می‌کند. همچنین مقدار محدودی از صف‌بندی را معرفی می‌کند، به طوری که در مواردی از شکست درخواست‌ها در اوج بسیار کوتاه جلوگیری شود. درخواست‌ها از صف‌ها با استفاده از یک تکنیک صف بندی عادلانه ارسال می‌شوند، به طوری که به عنوان مثال یک کنترل‌کننده بد رفتارانه نباید دیگران را از گردندرمانی کند (حتی در سطح اولویت یکسان).

این ویژگی طراحی شده است تا با کنترل‌کننده‌های استاندارد کار کند که از informers استفاده می‌کنند و به شکست‌های درخواست‌های API با exponential back-off واکنش می‌دهند، و دیگر مشتریانی که همین طور کار می‌کنند.

{{< caution >}}
برخی از درخواست‌ها که به عنوان "درخواست‌های بلندمدت" دسته‌بندی می‌شوند - مانند اجرای دستور از راه دور یا دنبال کردن لاگ - مشمول فیلتر APF نیستند. این وضعیت برای پرچم `--max-requests-inflight` بدون فعال بودن ویژگی اولویت و عدالت API هم صادق است. اما درخواست‌های **watch** تحت محدودیت `--max-requests-inflight` قرار دارند.
{{< /caution >}}

<!-- body -->

## فعال‌یا غیرفعال‌سازی اولویت و عدالت API

ویژگی اولویت و عدالت API توسط یک پرچم خط فرمان کنترل می‌شود و به طور پیش‌فرض فعال است. برای توضیح کلی در مورد پرچم‌های خط فرمان kube-apiserver و چگونگی فعال و غیرفعال کردن آن‌ها، به
[Options](/docs/reference/command-line-tools-reference/kube-apiserver/#options)
مراجعه کنید. نام گزینه خط فرمانی برای APF "--enable-priority-and-fairness" است. این ویژگی همچنین شامل یک {{< glossary_tooltip term_id="api-group" text="گروه API" >}}
است با: (a) نسخه پایدار `v1`، که در نسخه 1.29 معرفی شده و به طور پیش‌فرض فعال است (b) نسخه `v1beta3`، که به طور پیش‌فرض فعال است و در v1.29 قدیمی شده است. شما می‌توانید نسخه بتای گروه `v1beta3` را با اضافه کردن پرچم‌های خط فرمان زیر به فراخوانی `kube-apiserver` خود غیرفعال کنید:

```shell
kube-apiserver \
--runtime-config=flowcontrol.apiserver.k8s.io/v1beta3=false \
 # …و سایر پرچم‌های خط فرمان به صورت معمول
```

پرچم خط فرمان `--enable-priority-and-fairness=false` ویژگی اولویت و عدالت API را غیرفعال می‌کند.

## سناریوهای سرور بازگردانده

اولویت و عدالت API باید با دقت در سناریوهای بازگردانده سرور استفاده شود. این سناریوها شامل وضعیت‌هایی است که در آن‌ها برخی سرور A، در حال خدمت دادن به یک درخواست، یک درخواست فرعی به برخی سرور B صادر می‌کند. شاید سرور B حتی درخواست فرعی بیشتری به سرور A صادر کند. در موقعیت‌هایی که کنترل اولویت و عدالت بر روی هر دو درخواست اصلی و برخی از زیرمجموعه‌های آن اعمال می‌شود، مهم نیست چقدر در چشمه بازگردانده اساسی و برخی از درخواست‌های کنترلی ناقص درهم گرفته شده باشند.

یک مثال از بازگردانده این است که `kube-apiserver` یک تماس webhook اجازه راه B را صادر کند، و در حال خدمت به آن تماس، B سرور یک درخواست فرعی دیگر را به kube-apiserver صادر می‌کند. یک مثال دیگر از بازگرداندن ا

ین است که یک APIService برای انتقال درخواست‌ها در مورد یک گروه API خاص به یک سرور خارجی سفارشی B kube-apiserver را مستقیم می‌کند (این یکی از چیزهایی است که بازرجی نامیده می‌شود).

هنگامی که مشخص است که درخواست اصلی به سطح اولویت خاصی تعلق دارد و درخواست‌های کنترلی که کنترل می‌شوند، به سطوح اولویت بالاتر اختصاص می‌یابند، این یک راه حل ممکن است. وقتی که درخواست‌های اصلی می‌توانند به هر سطح اولویتی تعلق داشته باشند، درخواست‌های کنترلی که کنترل می‌شوند، باید از محدودیت‌های اولویت و عدالت مستثنی شوند. یکی از راه‌های انجام این کار با اشیاءی است که دسته‌بندی و برخورد کردن را پیکربندی می‌کنند، که در زیر بحث شده است. راه دیگری برای غیرفعال کردن اولویت و عدالت بر روی سرور B به طور کامل استفاده از تکنیک‌های بالا. راه سوم، که ساده‌ترین است برای استفاده از آن هنگامی که سرور B `kube-apisever` نیست، این است که سرور B را با اولویت و عدالت غیرفعال کنید.

## مفاهیم

چندین ویژگی متمایز در ویژگی اولویت و عدالت API وجود دارد. درخواست‌های ورودی با استفاده از ویژگی‌های درخواست، با استفاده از _FlowSchemas_ طبقه‌بندی شده و به سطوح اولویت اختصاص داده می‌شوند. سطوح اولویت با حفظ حداقل محدودیت‌های همزمانی جداگانه، جلوی گرسنگی‌های یکدیگر را برای درخواست‌هایی که به سطوح اولویت مختلف اختصاص داده شده‌اند، می‌گیرند. در هر سطح اولویت، الگوریتم صف بندی عادلانه جلوی گرسنگی از جریان‌های مختلف را بر می‌گرداند و اجازه می‌دهد تا درخواست‌ها در صف قرار گیرند تا از اینکه ترافیک ناگهانی باعث شود درخواست‌های انجام نشده رخ دهد، جلوگیری شود زمانی که بارگیری متوسط مورد پذیرش قرار گیرد.

### سطوح اولویت

بدون APF فعال، کل همزمانی در سرور API توسط پرچم‌های kube-apiserver محدود شده است `--max-requests-inflight` و `--max-mutating-requests-inflight`. با فعال بودن APF، محدودیت‌های همزمانی تعریف شده توسط این پرچم‌ها جمع‌آوری شده و سپس مجموع آن بین مجموعه قابل پیکربندی از _سطوح اولویت_ تقسیم می‌شود. هر درخواست ورودی به یک سطح اولویت اختصاص می‌یابد و هر سطح اولویت فقط تعداد درخواست‌های همزمان را ارسال می‌کند که مجوز خاص آن را می‌دهد.

پیکربندی پیش‌فرض، به عنوان مثال، شامل سطوح اولویت جداگانه برای درخواست‌های انتخاب رهبر، درخواست‌های از کنترل‌کننده‌های داخلی و درخواست‌های از Pods است. این بدان معناست که یک Pod بد رفتار که سرور API را با درخواست‌های زیاد غرق می‌کند، نمی‌تواند از موفقیت انتخاب رهبر یا اقدامات کنترل‌کننده‌های داخلی جلوگیری کند.

محدودیت‌های همزمانی سطوح اولویت به طور دوره‌ای تنظیم می‌شوند، اجازه می‌دهند تا سطوح اولویت کم استفاده‌شده، موقتاً هم‌زمانی را به سطوح شدیداً استفاده‌شده قرض دهند. این محدودیت‌ها بر اساس حدود حداقلی و محدوده‌هایی که چقدر می‌توانند هم‌زمانی را به ارث برده و چقدر می‌توانند قرض بگیرند، همگی از اشیاء پیکربندی شده از این پایین گرفته شده است.
